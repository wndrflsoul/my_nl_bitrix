...
$arParams = $handler->validateInputParams($arParams);
$arParams = $handler->fetchIBlockIds($arParams);
...
if (empty($arParams["PROPERTY_CODE"]) || !is_array($arParams["PROPERTY_CODE"]))
    $arParams["PROPERTY_CODE"] = array(); foreach ($arParams["PROPERTY_CODE"] as $key => $val)
    if ($val === "")
...
if ($this->startResultCache(false, array(($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters))) {
    ...
    $arResult["ITEMS"] = $handler->groupItemsByIBlockId($arResult["ITEMS"]);
